...
export default class Home extends Component {
  ...
  showModal = async () => {
    this.setState({
      ...
      analysisDetailsLabel:
        this.state.percentages.p1 !== undefined
          ? ` - 腐食率% =${Math.round(this.state.percentages.p1)}%`
          : ``,
    });
  };
  showModal2 = async () => {
  ...
  renderTab = (tabName) => {
    return (
      <Tab.Pane>
        {tabName === "PickImage" && this.renderImagePicker()}
        {tabName === "Realtime" && this.renderRealtimeInference()}
      </Tab.Pane>
    );
  };
      ...
      <div>
        <Grid columns={2} style={{ width: "100%" }} textAlign="center" celled>
          <Grid.Row>
            ...
            <Grid.Column>
              {analyzedFileUrl && (
                <h5> 検出結果 - 腐食率% ={Math.round(percentages.p1)}%</h5>
              )}
              {!analyzedFileUrl && <h5> 検出結果 </h5>}
            </Grid.Column>
          </Grid.Row>
          <Grid.Row>
            <Grid.Column
            ...
            >
              {analyzedFileUrl && (
                <img alt="" src={analyzedFileUrl} className="uploaded-image" />
              )}
    ...
    const panes = [
      {
        menuItem: {
          key: 1,
          content: "画像選択",
          icon: <Icon name="image" color="green"></Icon>,
        },
      ...
      },
      {
        menuItem: {
          key: 2,
          content: "リアルタイム",
          icon: <Icon name="camera" color="orange"></Icon>,
        },
    ...
    return (
      <OneColumnLayout>
        <Header as="h3">
          ...
          <Header.Content>オンデマンド検出</Header.Content>
        </Header>
        <hr />
